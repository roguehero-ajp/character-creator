...
| State | Areas |
|---|---|
| Playable | All numbered surface Areas 1–12; western junction; provisional sewer scaffold; Tavern; General Store |
| Planned | Final multi-part sewer design; hidden-route discovery and traversal logic |

The complete Briarwell surface is now playable. It contains the northern circuit
`1 → 2 → 3 → 4 → 1`, the southern circuit
`1 → 5 → 7 → 8 → 9 → 6 → 1`, Mayor's Hill from Area 3, and the western branch
`1 → 5 → west junction → 12`. Area 11 has finished runtime art and geometry but
remains reachable only through future hidden-route interaction logic.
...
| 4 | `briarwell-library-quarter` | Library, houses, cliffside alley | 1 SW, 3 W | Open window to 11; alley grate to sewers |
...
| 7 | `briarwell-ainsley-church` | Ainsley's House for Unwanted Children, church | 5 N by alley, 8 E | Alley grate to sewers |
| 8 | `briarwell-south-gate` | Guards, barracks, city gate, house | 1 N, 7 W, 9 E; road out of town S | — |
| 9 | `briarwell-docks` | Docks, fisherman's house | 6 N, 8 W | Dockside sewer access |
| 10 | `briarwell-mayors-hill` | Mayor's house on the hill | 3 S | — |
| 11 | `briarwell-blight-orphanage` | Ms. Blight's orphanage, barn | None | Open window to Area 4 alley |
...
| `briarwell-sewers` | Provisional playable scaffold; final network will be more complex | Hidden access at Areas 1, 4, 7 and 9 |
...
7. ~~Provisional sewer scaffold and four access anchors.~~ Complete for testing; not final.
8. Final sewer network design, hidden-route discovery and two-way traversal logic.
