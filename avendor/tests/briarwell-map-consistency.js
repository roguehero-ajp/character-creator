...
function hasReachableInteractionPoint(map, reachability, feature) {
  ...
  return sampleRadii.some((sampleRadius) => {
    for (let index = 0; index < 24; index += 1) {
      ...
      const point = {
        x: target.x + (Math.cos(angle) * sampleRadius),
        y: target.y + (Math.sin(angle) * sampleRadius)
      };
...
function assertMapConsistency(MapGeometry, data) {
  ...
  data.interactables.forEach((feature) => {
    assert(
      hasReachableInteractionPoint(map, reachability, feature),
      `Interactable has no reachable approach: ${data.id}/${feature.id}`
    );
  });

  data.npcs.forEach((npc) => {
    assert(
      hasReachableInteractionPoint(map, reachability, {
        ...npc,
        radius: npc.interactionRadius,
        interactionTarget: {
          x: npc.interactionX ?? npc.x,
          y: npc.interactionY ?? npc.y
        }
      }),
