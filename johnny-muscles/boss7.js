...
const MAX_HP=20;
let last=performance.now(),running=false,over=false,win=false,score=0,health = window.JMPowerUps?.getStartingState().health ?? 5,hp=MAX_HP,elapsed=0,steroids = window.JMPowerUps?.getStartingState().steroids ?? 3,steroidTimer = window.JMPowerUps?.getStartingState().steroidTimer ?? 0,muted=false,audio=null,aim=null,shake=0,toastTimer=null,hits=0,catches=0;
...
function hurt(t){if(over||win)return;health = window.JMPowerUps?.takeDamage(health) ?? (health - 1);shake=22;impact(155,GROUND-88,true,'#ffac7d');toast(t,1050);beep('hurt');hud();if(health<=0){running=false;over=true;finalScoreEl.textContent=`Dina HP: ${hp}/${MAX_HP} · Overhead catches: ${catches}`;overScreen.classList.add('visible')}}
...
function reset(){running=true;over=win=false;score=0;health = window.JMPowerUps?.getStartingState().health ?? 5;hp=MAX_HP;elapsed=0;steroids = window.JMPowerUps?.getStartingState().steroids ?? 3;steroidTimer = window.JMPowerUps?.getStartingState().steroidTimer ?? 0;aim=null;shake=0;hits=catches=0;dinaState='idle';stateTimer=0;stateMax=1;grabClock=3.5;reachProgress=turnProgress=0;particles.length=floaters.length=0;Object.assign(hero,{armAngle:-.75,releaseTimer:0,torsoLean:0,squat:0,catchPose:0});resetTank(false);grabClock=3.5;steroidButton.disabled=false;steroidCountEl.textContent = window.JMPowerUps?.doseLabel(steroids) ?? '3 doses';hud();toast(`DINA IS TOO TALL FOR THE CAMERA. DO NOT LET HER STEAL THE ${F.projectileName}.`,2300)}
...
steroidButton.addEventListener('click',()=>{if(!running||steroids<=0)return;steroids--;steroidTimer=12;steroidCountEl.textContent=steroids===1?'1 dose':`${steroids} doses`;steroidButton.disabled=steroids<=0;toast('UNREGULATED STRENGTH!');beep('power')});
