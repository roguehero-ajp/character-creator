(() => {
  ...
  class MountainGame {
    constructor(){
      ...
      this.lastTime=performance.now();this.running=false;this.gameOver=false;this.missionComplete=false;this.score=0;this.health = window.JMPowerUps?.getStartingState().health ?? 5;this.elapsed=0;this.steroidsLeft = window.JMPowerUps?.getStartingState().steroids ?? 3;this.steroidTimer = window.JMPowerUps?.getStartingState().steroidTimer ?? 0;this.muted=false;this.audioCtx=null;this.aim=null;this.shake=0;this.cameraX=0;this.toastTimer=null;
    ...
    hurtHero(label){if(this.gameOver||this.missionComplete)return;this.health = window.JMPowerUps?.takeDamage(this.health) ?? (this.health - 1);this.damageTaken++;this.shake=20;this.addImpact(this.HERO_HIT_X,this.HERO_HIT_Y,true,'#f8d7ba');this.showToast(label,900);this.beep('hurt');this.updateHud();if(this.health<=0)this.endGame();}
