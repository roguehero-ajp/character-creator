(() => {
  ...
  let lastTime=performance.now(),running=false,gameOver=false,victory=false,score=0,health = window.JMPowerUps?.getStartingState().health ?? 5,bossHealth=BOSS_MAX_HP,bossPhase=1,bossX=1050,bossDir=1,bossHitFlash=0;
  let elapsed=0,steroidsLeft = window.JMPowerUps?.getStartingState().steroids ?? 3,steroidTimer = window.JMPowerUps?.getStartingState().steroidTimer ?? 0,muted=false,audioCtx=null,aim=null,shake=0,cameraX=0,toastTimer=null;
  ...
  function resetFight(){running=true;gameOver=false;victory=false;score=0;health = window.JMPowerUps?.getStartingState().health ?? 5;bossHealth=BOSS_MAX_HP;bossPhase=1;bossX=1050;bossDir=1;bossHitFlash=0;elapsed=0;steroidsLeft = window.JMPowerUps?.getStartingState().steroids ?? 3;steroidTimer = window.JMPowerUps?.getStartingState().steroidTimer ?? 0;aim=null;shake=0;cameraX=0;attackTimer=2.2;bossWindup=0;firstCatchPrompt=true;catches=0;misses=0;bossHits=0;catchHolding=false;catchPointer=null;incoming.active=false;particles.length=0;floaters.length=0;Object.assign(johnny,{armAngle:-.75,releaseTimer:0,torsoLean:0,squat:0,catchPose:0});resetPlayerTank(true);steroidButton.disabled=false;steroidCountEl.textContent = window.JMPowerUps?.doseLabel(steroidsLeft) ?? '3 doses';updateHud();showToast('BOSS 1: SEARGENT BOB THE BOBCAT',1400);}
  ...
  function missIncoming(){incoming.active=false;catchHolding=false;catchPointer=null;misses++;health = window.JMPowerUps?.takeDamage(health) ?? (health - 1);shake=20;addImpact(175,GROUND-90,true);showToast('TANKED! HOLD IT NEXT TIME!',1100);beep('hurt');updateHud();if(health<=0){loseFight();return;}attackTimer=phaseSettings().cooldown*.72;}
